...
func _build_ui() -> void:
	...
	_bindings_tree.custom_minimum_size.y = 92.0
	_bindings_tree.columns = 3
	_bindings_tree.column_titles_visible = true
	_bindings_tree.set_column_title(0, "目标 / 事件")
	_bindings_tree.set_column_title(1, "处理函数")
	_bindings_tree.set_column_title(2, "状态")
	_bindings_tree.set_column_expand(0, true)
	_bindings_tree.set_column_expand(1, true)
	_bindings_tree.set_column_expand(2, false)
	_bindings_tree.set_column_custom_minimum_width(2, 72)
	...
	_event_picker.custom_minimum_size.x = 124.0
	_event_picker.item_selected.connect(_on_event_selected)
	target_row.add_child(_event_picker)
	...
	_method_edit.placeholder_text = "处理函数"
	...
	_capture_check.text = "捕获"
	_capture_check.tooltip_text = "在事件冒泡到目标之前调用处理函数"
	method_row.add_child(_capture_check)
	_add_button = Button.new()
	_add_button.text = "连接事件"
...
func _populate_bindings() -> void:
	...
	for row: Dictionary in _model.get("bindings", []):
		...
		var event_label := "%s · %s" % [row.get("target", ""), row.get("event", "")]
		if not bool(row.get("enabled", true)):
			event_label = "[停用] %s" % event_label
		item.set_text(0, event_label)
		item.set_text(1, str(row.get("handler", "")))
		item.set_text(2, str(row.get("status", "")))
		item.set_metadata(0, int(row.get("index", -1)))
		item.set_metadata(1, bool(row.get("enabled", true)))
		item.set_tooltip_text(0, "事件常量：FGUIEvents.%s%s" % [
...
func _populate_events() -> void:
	...
	for event: Dictionary in target.get("events", []):
		...
		_event_picker.add_item("%s (%s)" % [event.get("label", ""), event.get("constant", "")])
...
func _on_binding_selected() -> void:
	...
	if has_selection:
		_toggle_button.text = "停用绑定" if bool(selected.get_metadata(1)) else "启用绑定"
