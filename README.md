...
- 可把预览中的指定 FUI 对象显式暴露为 `FGUITarget` 场景节点，在 Inspector 配置 ShaderMaterial，并挂载由 TSCN 持有的 Godot 特效和附件节点。
...
`FGUIView` 的运行时子节点不会保存进 `.tscn`。需要在某个 FUI 图片或控件上添加 Godot 材质、粒子或动画时：

1. 打开 GUI 预览；直接双击 `.fui` 时，插件会关联当前场景中唯一匹配的 `FGUIView`，存在多个匹配项时先在场景树选中目标 View。
2. 在层级树或画布中选择目标。
3. 点击“暴露节点”。
4. 插件会通过 Undo/Redo 在当前 `FGUIView` 下创建一个 `FGUITarget` 并自动选中。
5. 在 `material_override` 中拖入 `ShaderMaterial`，或把普通 Godot 节点添加为 `FGUITarget` 子节点。

`FGUITarget` 保存 package/component ID 和 FUI child ID 链，不保存临时 Godot `NodePath`。FUI 重新导入或预览重建后会自动重新定位并应用材质。直接添加到 `FGUITarget` 下的 Godot 节点仍由场景持有，默认在渲染层级中作为对应 FUI 对象的真实子节点，因此会保持目标的绘制顺序、祖先裁剪、变换和可见性，同时场景树父子关系和 `$N1Target/Particles` 路径保持不变。

`attachment_mode` 可切换为 `Overlay`，恢复到完整 FUI 视图上方的覆盖层行为；`attachment_behind_target` 可让附件绘制在目标本身后面。插件只调整 `FGUITarget` 的 CanvasItem 渲染父级，不会重挂或复制用户节点。

该功能不会展开或接管 FairyGUI 节点树，也不会改变 Controller、Gear、Relation、Transition、列表和事件行为。虚拟列表单元等回收对象不能作为持久暴露目标。完整约束见 [节点暴露桥接规范](docs/TARGET_BRIDGE_SPEC.md)。
...
生成文件与业务脚本完全分离。导入器分别记录运行时内容哈希和绑定接口哈希：FairyGUI 全量发布、图片/文本/布局等非接口变化只刷新资源与预览，不会重新生成或重新扫描 GDScript；只有公开组件、命名节点、Controller、Transition、类型或依赖关系变化时才合并触发一次代码生成。生成失败会保留上一份可用结果；手工 `FGUIObjectFactory.set_extension()` 始终优先于自动注册。导出插件只在绑定清单过期时生成，并会显式加入注册表和生成脚本，兼容仅导出所选场景依赖的发布配置。建议将生成目录纳入版本管理，使 CI 和正式构建使用确定的绑定代码。
