...
- Inspector 中的 `Open Preview` 也可以打开当前 `FGUIPackageResource` 或 `FGUIView`。
...
- 在 `FGUIPackageResource` 或 `FGUIView` Inspector 中点击 `Generate Bindings`。
...
func _on_item_clicked(event: FGUIEvent) -> void:
	print(event.data)
```
