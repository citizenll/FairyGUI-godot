...
- 第一阶段不把 Godot 附件节点重挂到 FUI 的真实 native child 下。
- 第一阶段不保证附件自动继承 ScrollPane、Mask 或组件内部裁剪；附件采用场景覆盖层语义。
...
`FGUITarget` 在编辑器和运行时同步目标的全局变换、尺寸和可见性。附件保留在代理节点下，作为 FUI 上方的场景覆盖层绘制。

禁止在编辑器中把 scene-owned 附件重挂到临时 FUI native node 下，因为这会破坏 `.tscn` ownership 和保存路径。需要真实内部裁剪或插入 FUI display list 的能力时，后续应使用显式 `PackedScene` 实例化模式实现。
...
没有 `FGUIView` 上下文、组件不一致、目标属于动态列表内容或无法建立稳定引用时，“暴露节点”必须禁用并说明原因。
...
- 代理附件默认位于完整 FUI 视图上方；需要精确内部层级时必须使用未来的显式插入模式。
...
3. 清除或更换目标后原对象材质恢复。
4. `FGUITarget` 子节点在 FUI 刷新后仍由场景持有且变换继续同步。
