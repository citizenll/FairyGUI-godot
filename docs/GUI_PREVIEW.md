...
重新加载前会按资源路径和组件名保存节点选择、树折叠状态、筛选文本、缩放和滚动位置；新组件构建完成后使用组件内索引路径恢复，不持有已经释放的 FairyGUI 对象。
...
- 包内组件选择器，默认优先选择 `Main`。
- 重新加载。
- 从 `FGUIView` 打开时，将所选预览对象定位到 Inspector 事件绑定面板。
- 从 `FGUIView` 打开时，把稳定的静态 FUI 对象暴露为场景中的 `FGUITarget`。
- 缩小、放大和适应窗口。
...
- FairyGUI 组件可以从层级树拖到 Godot 2D 画布，创建对应包和组件的 `FGUIView`。

右侧区域使用比层级树更浅的编辑器背景色，与左侧深色节点列表形成明确分区；渲染本身使用与 `FGUIView` 相同的 `.fui` 构建链路，不创建另一套预览实现。

右侧视口的操作与 Godot 2D 编辑器保持一致：按住鼠标中键拖动画布，滚动鼠标滚轮缩放。滚轮缩放会保持鼠标指向的 FairyGUI 坐标不移动，工具栏的放大和缩小按钮则以视口中心为缩放中心。
...
从 `FGUIView` Inspector 打开预览时，“绑定事件”会把当前对象的真实名称路径传回同一场景节点，并在 Inspector 中自动选中该目标。直接双击 `.fui` 时，插件优先使用场景树当前选中的匹配 `FGUIView`；未选中时仅在当前场景恰好存在一个相同包和组件的 View 时自动关联。存在多个匹配项时必须先选中目标 View，避免把绑定写入错误场景节点。

“暴露节点”使用 package/component ID 与 child ID 链创建 `FGUITarget`，允许在场景中配置材质和添加 Godot 附件。附件默认保留所选 FUI 对象的 CanvasItem 渲染层级，但自身仍保持在 `FGUITarget` 的场景树层级中；需要全局覆盖效果时可把 `attachment_mode` 改为 `Overlay`。动态列表 Item 或无法建立稳定引用的对象不会开放该操作。重复暴露同一对象时选择已有代理，不创建重复节点。
