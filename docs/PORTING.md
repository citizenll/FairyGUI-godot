...
- Scale9, common widgets, list layout, scroll containers, bitmap fonts, audio, and pixel hit testing are present. `GImage` and `GLoader` support horizontal, vertical, Radial90, Radial180, and Radial360 fill meshes, including progress-bar fill amount updates. Synthetic pixel probes now cover fill wedges, forward/reversed shape masks, and blend output across Forward+/Mobile/Compatibility renderers. Complex transition timelines, very large virtual lists, real-project visual baselines, and performance still require commercial-freeze parity work.
