...
- `GRoot` tracks Godot Viewport resizing, manages nested popup stacks, automatic up/down placement, target-relative coordinates, outside-pointer dismissal, configured/custom tooltip windows, and FairyGUI-to-Godot input focus. Objects fall back to Godot native tooltip text when no FairyGUI tooltip resource is configured.
