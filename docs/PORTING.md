...
- FairyGUI blend modes `Normal`, `None`, `Add`, `Multiply`, `Screen`, `Erase`, `Mask`, `Below`, `Off`, and `One_OneMinusSrcAlpha` map to native Godot blending or destination-aware canvas shaders and remain active through color-filter/gray rendering. Custom blend slots 1-3 currently fall back to normal blending.
...
- Virtual lists support single-row, single-column, flow-horizontal, flow-vertical, and pagination layouts with pooled-item recycling, logical item renderer/provider indices, visible selection state, `scroll_to_view` visibility/`set_first` behavior, runtime `virtual_item_size`/gap layout refreshes, and horizontal/vertical content alignment. Scroll updates retain overlapping cells and render only newly visible items; forced refreshes reuse matching pooled resources. Single-row and single-column lists store renderer-provided variable sizes as sparse chunked overrides, so uniform million-item lists do not allocate per-item layout records. `invalidate_virtual_item_size` and `invalidate_virtual_item_sizes` reset stale measurements after model changes. Six-copy row/column/pagination loops remain bounded to viewport object counts; flow layouts remain uniform-cell and do not support loop mode.
...
- `FGUIEventDispatcher` implements direct, capture/bubble, and broadcast delivery with pooled `FGUIEventContext` values, standard `onClick`/`onChanged` event names, propagation cancellation, default prevention, native input details, and pointer capture outside the original Control. The compatibility `on`/`off` API still receives raw payloads; `add_event_listener` receives an `FGUIEventContext`.
- Buttons, component hit areas, drag operations, sliders, combo boxes, scroll bars, and window focus handling accept Godot screen-touch input in addition to mouse input. Object drags use configurable mouse/touch thresholds, root-relative bounds, and a global input relay; `FGUIDragDropManager` supplies a touch-disabled drag agent and dispatches FairyGUI-compatible drop data with the source object as `EventContext.initiator`.
...
- Components support forward and reversed image, scale9/atlas, rectangle, rounded rectangle, ellipse, and polygon alpha masks. Forward masks use native `CanvasItem` clipping; reversed masks use local destination-aware shaders with cached, resolution-bounded mask textures. Native Godot `Control` hit testing follows graph-mask geometry, component opacity, and configured child hit targets.
...
- Transition XY paths support straight, quadratic/cubic Bezier, and Catmull-Rom segments with FairyGUI-compatible start offsets. Package-version-4 custom easing paths are sampled and used by transitions and `GTweener`. Active transitions propagate pause/resume and zero-or-positive time-scale changes to nested transitions and animation targets, preserve animation state across repeated pause listeners, and can run manually while the owner is off-stage. Repeated/yoyo/infinite items honor finite end-time breakpoints without firing end hooks, zero color-filter endpoints release their shader state, and stopping with completion finishes only currently scheduled actions while leaving an initial delayed timeline untouched. Skew actions remain intentionally out of scope for the current milestone.
...
- Scale9, tiled images, common widgets, list layout, scroll containers, bitmap fonts, audio, and pixel hit testing are present. `GImage` and `GLoader` support horizontal, vertical, Radial90, Radial180, and Radial360 fill meshes, including progress-bar fill amount updates. Synthetic pixel probes cover fill wedges, forward/reversed shape masks, and blend output across Forward+/Mobile/Compatibility renderers. Million-item list stress is covered headlessly; real-project visual baselines and target-device performance still require commercial-freeze validation.
