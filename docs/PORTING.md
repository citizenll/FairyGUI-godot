...
- Buttons synchronize Up/Over/Down/Selected/Disabled controller pages, honor controller automatic radio-group depth, support tint/scale down effects, and toggle linked popups or windows on pointer press.
...
- Transition XY paths support straight, quadratic/cubic Bezier, and Catmull-Rom segments with FairyGUI-compatible start offsets. Active transitions propagate pause/resume and time-scale changes to nested transitions and animation targets. Skew actions remain intentionally out of scope for the current milestone.
